...
//
// ------------------------------------------------------------
// 【第三方代码声明】Apache License 2.0 第 4(b) 条
//
// 本文件中 RunningHub 应用接入、工具箱交互等相关部分的代码，
// 衍生自 PixelRunner / 小T修图助手（Copyright 2026 XIAOTsune，
// Apache License 2.0 授权），并已由本项目作者修改。
//
// 署名与许可证全文见同目录的 NOTICE、LICENSE-Apache-2.0.txt。
// 本项目整体按 GPL-3.0 分发。
// ============================================================
...
(function() {
    ...
    async function syncServerPresets(options) {
        ...
        // saveServerPresets 是把整个 config 重新 JSON.stringify 回写的，
        // 预设带 base64 参考图时很容易撑爆 localStorage 配额。不检查返回值的话，
        // 写盘失败了界面还是会说「已同步 N 条」，用户下次进来发现一条都没有。
        if (!Config.saveServerPresets(deduped)) {
            throw new Error('服务器预设写入失败（本地存储可能已满，本次 ' + deduped.length + ' 条）');
        }
        if (currentSettings) {
        ...
        return [
            {
                id: 'houdini-vfx',
                icon: 'VFX',
                title: 'VFX 特效（Houdini 粒子）',
                desc: '粒子类型、11 项特效与物理光学参数的独立 VFX。',
                editorType: 'houdini-vfx-ui'
            }
        ];
    }
    ...
    function renderAppsHome() {
        ...
        const appCountCard = '<div class="module-card field-stack">'
            ...
            + '<div class="info-text">内置功能：' + cards.length + ' 个。半合成固定显示；RunningHub 已独立到“应用”页面。</div>'
            + '</div>';
    ...
    referenceHoudiniEffects.forEach(function(effect) {
        referenceHoudiniState.effects[effect[0]] = { weight: effect[2], enabled: true };
    });

    function renderReferenceHoudiniEditor() {
        const title = document.getElementById('appEditorTitle');
        const desc = document.getElementById('appEditorDesc');
        const body = document.getElementById('appEditorBody');
        if (title) title.textContent = 'VFX 特效（Houdini 粒子）';
        ...
        const rangeRow = function(label, id, value, min, max, step, valueText, toggleKey, enabled) {
            return '<div class="houdini-control-card">'
                + '<div class="houdini-control-head"><span class="w10-row-label">' + escapeHTML(label) + '</span><span class="w10-ps-val" id="' + id + 'Val">' + escapeHTML(valueText) + '</span></div>'
                + '<div class="houdini-control-track"><input type="range" id="' + id + '" min="' + min + '" max="' + max + '" step="' + step + '" value="' + value + '"' + (enabled === false ? ' disabled' : '') + '>'
                + (toggleKey ? '<div class="w10-toggle' + (enabled === false ? '' : ' on') + '" data-houdini-toggle="' + toggleKey + '"></div>' : '') + '</div>'
                + '</div>';
        };

        const effectsHtml = referenceHoudiniEffects.map(function(effect) {
            const current = referenceHoudiniState.effects[effect[0]];
            return rangeRow(effect[1], 'houdiniFx_' + effect[0], current.weight, 0, 1, 0.01, current.weight.toFixed(2), 'effect:' + effect[0], current.enabled);
        }).join('');
        const houdiniRouteControls = buildIndependentGenerationControls('app:houdini-vfx', 'houdiniRoute', 'Houdini 粒子独立生成参数');

        body.innerHTML = ''
            + '<div class="module-card w10-panel houdini-panel">'
            + '  <div class="w10-section-title">粒子类型</div>'
            + '  <div class="houdini-two-column">'
            + '    <div class="houdini-select-card"><label>主粒子</label><select class="w10-select" id="houdiniMainType">' + renderOptions(referenceHoudiniState.mainType) + '</select></div>'
            + '    <div class="houdini-select-card"><label>辅粒子</label><select class="w10-select" id="houdiniSecondaryType">' + renderOptions(referenceHoudiniState.secondaryType) + '</select></div>'
            + '  </div>'
            + rangeRow('辅助混合占比', 'houdiniMix', referenceHoudiniState.mix, 0, 1, 0.01, Math.round(referenceHoudiniState.mix * 100) + '%', '', true)
            + '  <div class="w10-section-title">粒子特效（11 项）</div>'
            + '  <div class="houdini-effects-grid">' + effectsHtml + '</div>'
            + '  <div class="w10-section-title">物理 / 光学</div>'
            + '  <div class="houdini-effects-grid">'
            + rangeRow('折射率 IOR', 'houdiniIor', referenceHoudiniState.ior, 1, 2, 0.01, referenceHoudiniState.ior.toFixed(2), 'iorOn', referenceHoudiniState.iorOn)
            + rangeRow('噪波影响范围', 'houdiniNoiseRange', referenceHoudiniState.noiseRange, 0, 1, 0.01, referenceHoudiniState.noiseRange.toFixed(2), 'noiseOn', referenceHoudiniState.noiseOn)
            + rangeRow('噪波幅度（细密度）', 'houdiniNoiseAmp', referenceHoudiniState.noiseAmp, 0, 1, 0.01, referenceHoudiniState.noiseAmp.toFixed(2), '', referenceHoudiniState.noiseOn)
            + rangeRow('动态模糊（运动拖尾）', 'houdiniBlur', referenceHoudiniState.blur, 0, 1, 0.01, referenceHoudiniState.blur.toFixed(2), 'blurOn', referenceHoudiniState.blurOn)
            + rangeRow('菲涅尔边缘 / 表面蔓延', 'houdiniFresnelSpread', referenceHoudiniState.fresnelSpread, 0, 1, 0.01, referenceHoudiniState.fresnelSpread.toFixed(2), 'fresnelOn', referenceHoudiniState.fresnelOn)
            + rangeRow('重力强度', 'houdiniGravity', referenceHoudiniState.gravity, 0, 1, 0.01, referenceHoudiniState.gravity.toFixed(2), 'gravityOn', referenceHoudiniState.gravityOn)
            + '  </div>'
            + '  <div class="w10-section-title">生成</div>'
            + houdiniRouteControls
            + '  <div class="houdini-generation-grid">'
            + '    <div class="houdini-select-card"><label>宽高比</label><select class="w10-select" id="houdiniAspect"><option>Auto</option><option>1:1</option><option>3:2</option><option>2:3</option><option>16:9</option><option>9:16</option><option>4:3</option><option>3:4</option></select></div>'
            + '  </div>'
            + '  <div class="houdini-actions"><button class="w10-btn" id="houdiniRandomBtn" type="button">' + uiIconLabel('dice', 'Houdini 大爆炸（随机所有参数）') + '</button><div class="btn-row"><button class="w10-btn" id="houdiniDisableBtn" type="button">关闭所有粒子</button><button class="w10-btn w10-btn-accent" id="houdiniStartBtn" type="button">' + uiIconLabel('sparkle', '生成特效') + '</button></div><div class="w10-row-desc" id="houdiniStatus">先在 Photoshop 中框选需要生成特效的区域</div></div>'
            + '</div>';

        initAllFakeSelects(body);
        initAllRenderedRanges(body);
        bindIndependentGenerationControls('app:houdini-vfx', 'houdiniRoute', body);
        ...
        bindRange('houdiniMix', referenceHoudiniState, 'mix', function(value) { return Math.round(value * 100) + '%'; });
        referenceHoudiniEffects.forEach(function(effect) {
            bindRange('houdiniFx_' + effect[0], referenceHoudiniState.effects[effect[0]], 'weight', function(value) { return value.toFixed(2); });
        });
        bindRange('houdiniIor', referenceHoudiniState, 'ior', function(value) { return value.toFixed(2); });
        bindRange('houdiniNoiseRange', referenceHoudiniState, 'noiseRange', function(value) { return value.toFixed(2); });
        bindRange('houdiniNoiseAmp', referenceHoudiniState, 'noiseAmp', function(value) { return value.toFixed(2); });
        bindRange('houdiniBlur', referenceHoudiniState, 'blur', function(value) { return value.toFixed(2); });
        bindRange('houdiniFresnelSpread', referenceHoudiniState, 'fresnelSpread', function(value) { return value.toFixed(2); });
        bindRange('houdiniGravity', referenceHoudiniState, 'gravity', function(value) { return value.toFixed(2); });
        document.getElementById('houdiniMainType').onchange = function(event) { referenceHoudiniState.mainType = event.target.value; };
        document.getElementById('houdiniSecondaryType').onchange = function(event) { referenceHoudiniState.secondaryType = event.target.value; };
        body.querySelectorAll('[data-houdini-toggle]').forEach(function(toggle) {
            toggle.onclick = function() {
                const key = toggle.getAttribute('data-houdini-toggle');
                if (key.indexOf('effect:') === 0) {
                    const effectId = key.slice(7);
                    const state = referenceHoudiniState.effects[effectId];
                    state.enabled = !state.enabled;
                    const control = document.getElementById('houdiniFx_' + effectId);
                    if (control) control.disabled = !state.enabled;
                    toggle.classList.toggle('on', state.enabled);
                    return;
                }
                referenceHoudiniState[key] = !referenceHoudiniState[key];
                toggle.classList.toggle('on', referenceHoudiniState[key]);
                const slaves = {
                    iorOn: ['houdiniIor'], noiseOn: ['houdiniNoiseRange', 'houdiniNoiseAmp'],
                    blurOn: ['houdiniBlur'], fresnelOn: ['houdiniFresnelSpread'], gravityOn: ['houdiniGravity']
                }[key] || [];
                slaves.forEach(function(id) { const control = document.getElementById(id); if (control) control.disabled = !referenceHoudiniState[key]; });
            };
        });
        document.getElementById('houdiniRandomBtn').onclick = function() {
            referenceHoudiniState.mix = Math.random();
            referenceHoudiniEffects.forEach(function(effect) {
                referenceHoudiniState.effects[effect[0]].weight = Math.random();
                referenceHoudiniState.effects[effect[0]].enabled = Math.random() > 0.15;
            });
            referenceHoudiniState.ior = 1 + Math.random();
            referenceHoudiniState.noiseRange = Math.random();
            referenceHoudiniState.noiseAmp = Math.random();
            referenceHoudiniState.blur = Math.random();
            referenceHoudiniState.fresnelSpread = Math.random();
            referenceHoudiniState.gravity = Math.random();
            renderReferenceHoudiniEditor();
        };
        document.getElementById('houdiniDisableBtn').onclick = function() {
            referenceHoudiniEffects.forEach(function(effect) { referenceHoudiniState.effects[effect[0]].enabled = false; });
            renderReferenceHoudiniEditor();
        };
        document.getElementById('houdiniStartBtn').onclick = async function() {
            const enabledEffects = referenceHoudiniEffects.filter(function(effect) { return referenceHoudiniState.effects[effect[0]].enabled; }).map(function(effect) {
                return effect[1] + '（强度 ' + referenceHoudiniState.effects[effect[0]].weight.toFixed(2) + '）';
            });
            const prompt = '8K/4K 超高清细节，照片级真实渲染，Houdini 粒子系统。基于参考图绘制线条区域生成影视级 VFX；主粒子：' + document.getElementById('houdiniMainType').options[document.getElementById('houdiniMainType').selectedIndex].textContent + '；辅粒子：' + document.getElementById('houdiniSecondaryType').options[document.getElementById('houdiniSecondaryType').selectedIndex].textContent + '，混合占比 ' + Math.round(referenceHoudiniState.mix * 100) + '%。启用效果：' + (enabledEffects.join('、') || '关闭粒子，仅保留原图') + '。折射率 ' + referenceHoudiniState.ior.toFixed(2) + '，噪波范围 ' + referenceHoudiniState.noiseRange.toFixed(2) + '，噪波幅度 ' + referenceHoudiniState.noiseAmp.toFixed(2) + '，动态模糊 ' + referenceHoudiniState.blur.toFixed(2) + '，菲涅尔蔓延 ' + referenceHoudiniState.fresnelSpread.toFixed(2) + '，重力 ' + referenceHoudiniState.gravity.toFixed(2) + '。严格保留人物身份、姿态、面部和背景，仅在线条指示区域生成特效。';
            const route = readIndependentGenerationControls('app:houdini-vfx', 'houdiniRoute');
            await img2Img({ prompt: prompt, model: route.model, imageResolution: route.imageResolution, imageCount: route.imageCount, type: 'houdini-vfx', title: 'VFX 特效（Houdini 粒子）', button: this, idleButtonLabel: uiIconLabel('sparkle', '生成特效') });
        };
    }
    ...
    function openAppCategory(categoryId, meta) {
        ...
        if (categoryId === 'houdini-vfx') {
            appEditorMeta = meta || { id: 'houdini-vfx', title: 'VFX 特效（Houdini 粒子）', desc: '快速界面独立 VFX' };
            const title = document.getElementById('appEditorTitle');
            const desc = document.getElementById('appEditorDesc');
            const body = document.getElementById('appEditorBody');
            ...
            renderReferenceHoudiniEditor();
            setAppsEditorVisible('houdini-vfx');
            return;
        }
    ...
    let workspaceGenerationRoutes = (function() {
        let parsed = {};
        try {
            const raw = JSON.parse(localStorage.getItem(WORKSPACE_GENERATION_ROUTES_KEY) || '{}');
            parsed = raw && typeof raw === 'object' ? raw : {};
        } catch (error) {
            return {};
        }
        // VFX 小应用从 kao-vfx 改名为 houdini-vfx，而这个表的 key 就是 scope 字符串。
        // 不搬的话，用户在那个面板里单独选的平台/模型/分辨率/数量读不到，
        // 看起来像是「参数自己复位了」。只在新 key 还不存在时搬一次。
        // 旧 key 保留不删 —— 与配置层的其它迁移一致，不破坏原数据。
        if (parsed['app:kao-vfx'] && !parsed['app:houdini-vfx']) {
            parsed['app:houdini-vfx'] = parsed['app:kao-vfx'];
            try { localStorage.setItem(WORKSPACE_GENERATION_ROUTES_KEY, JSON.stringify(parsed)); } catch (error) {}
        }
        return parsed;
    })();
