...
try {
  ...
  if (runtime && runtime.health && runtime.health.documentCount > 0) {
    ...
    await evaluateCdt(uiDebug.wsdebugUrl, `(() => {
      ...
      const houdiniCard = document.querySelector('#appsHome .app-launch-card[data-app-category="houdini-vfx"]');
      let houdiniClickError = '';
      try {
        if (houdiniCard && typeof houdiniCard.onclick === 'function') houdiniCard.onclick({ preventDefault() {}, stopPropagation() {} });
        else houdiniCard?.click();
      } catch (error) { houdiniClickError = error && error.message ? error.message : String(error); }
      await new Promise((resolve) => setTimeout(resolve, 180));
      const houdiniAudit = {
        card: !!houdiniCard,
        bound: typeof houdiniCard?.onclick === 'function',
        clickError: houdiniClickError,
        ...
        panel: !!document.querySelector('#appEditorBody .houdini-panel'),
        effects: document.querySelectorAll('#appEditorBody input[id^="houdiniFx_"]').length,
        toggles: document.querySelectorAll('#appEditorBody [data-houdini-toggle]').length,
        ...
        generate: typeof document.getElementById('houdiniStartBtn')?.onclick === 'function'
      };
      const backAfterHoudini = document.getElementById('btnBackToGenericAppHome');
      if (backAfterHoudini && typeof backAfterHoudini.onclick === 'function') backAfterHoudini.onclick(); else backAfterHoudini?.click();
