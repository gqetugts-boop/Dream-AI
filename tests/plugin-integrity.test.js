...
// houdini 面板的 .houdini-two-column / .houdini-effects-grid 曾随 local-engines.css 一起被删，
// 结果整个面板静默退化成单列，没有任何报错。
const allCss = html + '\n' + externalCssText;
['houdini-two-column', 'houdini-effects-grid', 'houdini-generation-grid', 'houdini-select-card', 'houdini-control-card']
    .forEach(function (cls) {
        total++;
        if (allCss.indexOf('.' + cls) > -1) { pass++; return; }
        console.log('❌ .' + cls + ' 被 JS 使用但没有任何 CSS 定义（布局会静默失效）');
    });
console.log('   5 个 Houdini 面板布局类均有样式定义');
